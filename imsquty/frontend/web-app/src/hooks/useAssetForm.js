...
export const assetValidationSchema = yup.object().shape({
    ...
    status: yup
        ...
        .oneOf(['active', 'inactive', 'maintenance', 'retired'], 'Invalid status selected'),
    notes: yup
        .string()
        .optional()
 ...
 */
export const useAssetForm = (defaultValues) => {
    const { register, handleSubmit, formState: { errors, isSubmitting, isValid }, watch, reset, setValue, control, } = useForm({
 ...
 */
export const useAssetFormSubmit = (onSuccess) => {
