...
export const ticketValidationSchema = yup.object().shape({
  ...
  status: yup
    ...
    .oneOf(
      ['Open', 'In Progress', 'Pending Info', 'Resolved', 'Closed'],
 ...
 */
export const useTicketForm = (defaultValues?: Partial<TicketFormData>) => {
  const {
    register,
    handleSubmit,
...
export const useTicketFormSubmit = (
  onSuccess: (data: TicketFormData) => Promise<void>,
) => {
