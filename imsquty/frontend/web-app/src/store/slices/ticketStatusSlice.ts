  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || 'Failed to fetch ticket statuses',
      )
    }
  },
)
...
const ticketStatusSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchTicketStatuses.fulfilled, (state, action: any) => {
        state.loading = false
        state.statuses = action.payload.data || []
      })
      .addCase(fetchTicketStatuses.rejected, (state, action: any) => {
        state.loading = false
