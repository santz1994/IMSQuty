...
import { Division, divisionService } from '../../api/divisionService'
...
export const fetchActiveDivisions = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || 'Failed to fetch active divisions',
      )
    }
  },
)
...
const divisionSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(fetchDivisions.fulfilled, (state, action: any) => {
        state.loading = false
      ...
      .addCase(fetchDivisions.rejected, (state, action: any) => {
        state.loading = false
    ...
    builder
      ...
      .addCase(fetchActiveDivisions.fulfilled, (state, action: any) => {
        state.loading = false
        state.divisions = action.payload.data || []
      })
      .addCase(fetchActiveDivisions.rejected, (state, action: any) => {
        state.loading = false
        state.error = action.payload
      })
  },
