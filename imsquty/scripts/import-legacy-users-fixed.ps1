existsqu# Import users from legacy database to new IMSQuty structure
...
Write-Host "[OK] Legacy SQL file found: $legacySqlFile" -ForegroundColor Green
...
if ($userInserts.Count -eq 0) {
    Write-Host "[ERROR] No user INSERT statements found in itquty.sql" -ForegroundColor Red
    Write-Host ""
...
Write-Host "[OK] Found $($userInserts.Count) user-related insert statement(s)" -ForegroundColor Green
Write-Host ""
...
Write-Host "[OK] Legacy SQL imported" -ForegroundColor Green
Write-Host ""
...
if ([string]::IsNullOrWhiteSpace($tableExists)) {
    Write-Host "[ERROR] Users table not found in legacy database" -ForegroundColor Red
...
Write-Host "[OK] Users table exists in legacy database" -ForegroundColor Green
Write-Host ""
...
Write-Host "[OK] Found $legacyCount users in legacy database" -ForegroundColor Green
Write-Host ""

# Map and import legacy users to new structure
Write-Host "Mapping legacy users to new structure..." -ForegroundColor Yellow
Write-Host "- Mapping 'name' to 'username'" -ForegroundColor Gray
...
SELECT 
    ...
    l.email_verified_at,
    l.password,
    COALESCE(NULLIF(SUBSTRING_INDEX(l.name, ' ', 1), ''), 'User') as first_name,
    COALESCE(NULLIF(SUBSTRING_INDEX(l.name, ' ', -1), ''), 'Name') as last_name,
    'active' as status,
...
Write-Host ""
Write-Host "[SUCCESS] Users imported and mapped successfully!" -ForegroundColor Green
Write-Host ""
...
Write-Host ""
Write-Host "Note: All imported users have default 'user' role." -ForegroundColor Yellow
