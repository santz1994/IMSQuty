...
class MeetingRoomBookingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $conflict = MeetingRoomBooking::where('room_name', $validated['room_name'])
    ...
    public function extendTime(Request $request, $id)
    {
        ...
        // Check if meeting is ongoing or approved
        ...
        if ($conflict) {
            return response()->json([
                'success' => false, 
                'message' => 'Cannot extend: Room is booked from ' . $conflict->start_datetime->format('H:i')
            ], 400);
        }
        ...
        return response()->json([
            'success' => true,
            'message' => "Meeting extended by {$extendMinutes} minutes until " . $newEndTime->format('H:i'),
    ...
    public function receptionistDashboard()
    {
        ...
        foreach ($this->rooms as $roomName) {
    ...
    public function quickBooking(Request $request)
    {
        ...
        // Check for conflicts
        $conflict = \App\MeetingRoomBooking::where('room_name', $validated['room_name'])
            ->where('status', '!=', 'rejected')
