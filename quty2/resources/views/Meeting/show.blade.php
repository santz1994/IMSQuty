            ...
            <form id="extendTimeForm" action="{{ route('meeting-room-bookings.extend', $booking->id) }}" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <select name="new_end_time" id="new_end_time" class="form-control" required>
                            <option value="">-- Pilih Waktu --</option>
                            @php
                                $currentEndHour = $booking->end_datetime->format('H');
                                $currentEndMinute = $booking->end_datetime->format('i');
                                for($h = $currentEndHour; $h <= 22; $h++) {
                                    for($m = 0; $m < 60; $m += 30) {
                                        $time = sprintf('%02d:%02d', $h, $m);
                                        $currentTime = sprintf('%02d:%02d', $currentEndHour, $currentEndMinute);
                                        if ($time > $currentTime) {
                    ...
                    <div class="form-group">
                        <label for="extend_reason">
                            Alasan Perpanjangan / Extension Reason <span class="text-danger">*</span>
                        </label>
                        <textarea name="extend_reason" id="extend_reason" class="form-control" rows="3" required
