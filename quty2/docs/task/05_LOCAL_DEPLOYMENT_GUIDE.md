...
Dokumen ini menjelaskan **aplikasi apa saja yang dibutuhkan** untuk running microservices imsquty di komputer lokal, termasuk:
...
# 2. Clone imsquty microservices
git clone https://github.com/santz1994/imsquty-microservices.git
...
# Output:
# [+] Running 15/15
#  ✔ Network imsquty_network         Created
#  ✔ Container imsquty-mysql         Started
#  ✔ Container imsquty-redis         Started
#  ✔ Container imsquty-rabbitmq      Started
#  ✔ Container imsquty-auth-service  Started
#  ✔ Container imsquty-user-service  Started
#  ✔ Container imsquty-asset-service Started
#  ✔ Container imsquty-ticket-service Started
#  ✔ Container imsquty-api-gateway   Started
#  ... (more services)
...
# Output:
# NAME                        STATUS    PORTS
# imsquty-api-gateway          Up        0.0.0.0:8000->8000/tcp
# imsquty-auth-service         Up        8001/tcp
# imsquty-asset-service        Up        8003/tcp
# imsquty-ticket-service       Up        8004/tcp
# imsquty-mysql                Up        0.0.0.0:3306->3306/tcp
# imsquty-redis                Up        6379/tcp
# imsquty-rabbitmq             Up        0.0.0.0:5672->5672/tcp, 0.0.0.0:15672->15672/tcp
...
services:
  ...
  mysql:
    image: mysql:8.0
    container_name: imsquty-mysql
    environment:
      MYSQL_ROOT_PASSWORD: ${DB_ROOT_PASSWORD:-secret}
      MYSQL_DATABASE: ${DB_DATABASE:-imsquty_db}
      MYSQL_USER: ${DB_USERNAME:-imsquty_user}
      MYSQL_PASSWORD: ${DB_PASSWORD:-imsquty_pass}
    ports:
      - "${DB_PORT:-3306}:3306"
    volumes:
  ...
  rabbitmq:
    ...
    container_name: imsquty-rabbitmq
    environment:
  ...
  api-gateway:
    ...
    container_name: imsquty-api-gateway
    ports:
  ...
  auth-service:
    build:
      ...
      dockerfile: Dockerfile
    container_name: imsquty-auth-service
    environment:
      - DB_HOST=mysql
      - DB_DATABASE=auth_db
      - REDIS_HOST=redis
  ...
  user-service:
    build:
      ...
      dockerfile: Dockerfile
    container_name: imsquty-user-service
    environment:
      - DB_HOST=mysql
      - DB_DATABASE=user_db
      - REDIS_HOST=redis
  ...
  asset-service:
    build:
      ...
      dockerfile: Dockerfile
    container_name: imsquty-asset-service
    environment:
      - DB_HOST=mysql
  ...
  ticket-service:
    build:
      ...
      dockerfile: Dockerfile
    container_name: imsquty-ticket-service
    environment:
      - DB_HOST=mysql
  ...
  web-app:
    build:
      ...
      dockerfile: Dockerfile
    container_name: imsquty-web-app
    ports:
  ...
  portainer:
    ...
    container_name: imsquty-portainer
    ports:
      - "9000:9000"
    volumes:
...
cd ~/projects/imsquty-microservices
docker compose up -d
...
# Export database
docker compose exec mysql mysqldump -u root -p imsquty_db > backup.sql

# Import database
docker compose exec -T mysql mysql -u root -p imsquty_db < backup.sql
...
# Check network
docker network inspect imsquty-microservices_imsquty-network
...
# imsquty-api-gateway  2.5%    150MiB / 8GiB        1.9%    1.2MB/850KB
# imsquty-mysql        5.3%    450MiB / 8GiB        5.6%    5MB/2MB
# imsquty-redis        0.5%    10MiB / 8GiB         0.1%    100KB/50KB
...
docker run --rm -v imsquty-microservices_mysql_data:/data \
